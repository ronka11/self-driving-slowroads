# Self Driving Car in GTAV
Control a self driving car using AI in GTAV ingame

## Steps
### 1. lane detection
1. - [x] detect lanes from images
2. - [x] detect lanes from video inputs
3. - [x] detect lanes from live video inputs

### 2. online game
1. - [x] find an online game to use to test -> slowroads.io
2. - [x] capture screen using python
3. - [x] control online game car using python
4. - [x] control movement from editor to control online car game

### 3.apply AI
1. - [x] learn/revise fundamentals
2. - [x] learn relevant self driving stuff
3. - [x] apply it on online game car

### 4. GTAV 
1. - [ ] capture data from gtaV
2. - [ ] control movement using editor
3. - [ ] apply above AI to gta


## Important Links
- [Lane detection for a self-driving car using OpenCV](https://medium.com/analytics-vidhya/lane-detection-for-a-self-driving-car-using-opencv-e2aa95105b89)
- [Real-Time Lane Detection for Self-Driving Cars using OpenCV](https://www.labellerr.com/blog/real-time-lane-detection-for-self-driving-cars-using-opencv/)
- [Python Plays: Grand Theft Auto V- SentDex](https://www.youtube.com/playlist?list=PLQVvvaa0QuDeETZEOy4VdocT7TOjfSA8a)
- [How to Simulate a Self-Driving Car- Siraj Raval](https://www.youtube.com/watch?v=EaY5QiZwSP4)
- [Self-driving cars with Carla and Python -SentDex](https://www.youtube.com/playlist?list=PLQVvvaa0QuDeI12McNQdnTlWz9XlCa0uo)
- [Computer Vision and Perception for Self-Driving Cars (Deep Learning Course) - FreeCodeCamp](https://www.youtube.com/watch?v=cPOtULagNnI)
- [Reading game frames in Python with OpenCV - Python Plays GTA V](https://pythonprogramming.net/game-frames-open-cv-python-plays-gta-v/)
- [Advanced Lane Lines Github](https://github.com/Dt-Pham/Advanced-Lane-Lines)
